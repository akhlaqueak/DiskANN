...
int search_memory_index(diskann::Metric &metric, const std::string &index_path, const std::string &result_path_prefix,
                        ...
                        const uint32_t filtered_medoids)
{
    using TagT = uint32_t;
    // Load the query file
    T *query = nullptr;
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        auto prec =
            diskann::calculate_precision<LabelT>((uint32_t)query_num, query_result_ids[test_id].data(), recall_at,
                                                 query_location_to_labels, location_to_labels, filter_map);
...
int main(int argc, char **argv)
{
    ...
    po::options_description desc{
       auto st= program_options_utils::make_program_description("search_memory_index", "Searches in-memory DiskANN indexes")};
    try
    {
